...
provider "hcloud" {}
locals {
  kubeapi_endpoint   = var.disable_kubeapi_loadbalancer ? hcloud_server.control_plane.0.ipv4_address : hcloud_load_balancer.load_balancer.0.ipv4
...
resource "hcloud_firewall" "api-server" {
  count = var.control_plane_replicas > 1 ? 0 : 1
  ...
  apply_to {
    server = hcloud_server.control_plane.0.id
  }
